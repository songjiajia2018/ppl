...
parser = argparse.ArgumentParser(description = '''Conduct analysis based on mito-genotyping. Complete pipeline will result in a rds file 
                                 ...
                                 Example(10x): python ppl/ppl2_test.py -t 10 --split-sam10x --input-filelist -p -m -r --name ppl10x --input 10x_list;
                                 Example(smart-seq2): python ppl/ppl2_test.py -t 10 --input-filelist -p -m -r --name pplsmart --input smart_list;
                                 Example(indrops): python ppl/ppl2_test.py -t 10 --indrops --input-filelist -p -m -r --name pplindrops --input indrops_list;
                                 Example(only pileup): python ppl/ppl2_test.py -t 10 --input-filelist -r --name pileup --input mylist'''
                                 )
parser.add_argument('--input-filelist', required = False , action = 'store_true', help = 'Input files with related \
                    out-prefixes(seperated by comma e.g.:sample1,out_prefix1,celltype(for indrops)/annotation(for spliting sam)) from a file.')
...
parser.add_argument('--maxBP', required = False, default = 16569, type = int, help = ' Specified maximum length of mtDNA genome. \
...
parser.add_argument('--split-sam10x', required = False, action = 'store_true', help = 'split sam according to the celltype annotation, \
                    Input sam files must be tagged with "CB:Z:" for reads barcodes.')
parser.add_argument('--split-chr', required = False, default = 'MT', help = 'Reads in out sam will be filtered by the chromosome. \
                    (default: MT)')
...
parser.add_argument('--indrops', required = False, action = 'store_true', help = 'Merge AGCT and coverage files according to annotation')
...
mod10x_bool = params.split_sam10x
chr10x = params.split_chr
indrops_bool = params.indrops
...
def indropsmergefunction(celltype_tmp, merge_cell_list, project_tmpname, out_dir_name):
    ...
    cell_name = 'cell_merged' + str(merged_cell_number)
    indrops_dir_path = out_dir_name + '/' + project_tmpname + '/indrops_merged'
...
if __name__ == '__main__':
    ...
    if mod10x_bool:
        ...
        with open(out_dir_path + '/' + project_name + '/' + '10x_split_input_file_list.tsv', 'w') as OUT_TMP:
            for (fn, fp) in new_input_file_list:
                OUT_TMP.write(fn + ',' + fp + '\n')
        input_file = out_dir_path + '/' + project_name + '/' + '10x_split_input_file_list.tsv'
...
# indrops merge with multiprocess
    if indrops_bool:
        ...
        p = multiprocessing.Pool(pool_number)
        os.system('mkdir {0}/{1}/indrops_merged'.format(out_dir_path, project_name))
...
# merge start
    if merge_pileup_bool:
        ...
        if indrops_bool:
            input_dir_merge = out_dir_path + '/' + project_name + '/indrops_merged'
